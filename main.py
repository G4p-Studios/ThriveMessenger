...
class ClientApp(wx.App):
    ...
    def on_file_declined(self, msg):
        ...
        chat = self.frame.get_chat(to)
        if chat: 
            chat.append(f"{to} declined your file(s): {names}", "System", get_utc_timestamp())
        else:
            wx.MessageBox(f"{to} declined your file(s): {names}", "File Declined", wx.ICON_INFORMATION)
    ...
    def on_file_data(self, msg):
        ...
        if saved:
            ...
            names = ", ".join(saved)
            if chat:
                chat.append(f"{len(saved)} file(s) received and saved: {names}", "System", get_utc_timestamp())
            else:
...
class MainFrame(wx.Frame):
    ...
    def receive_message(self, msg):
        ...
        if not is_contact and msg["from"] not in self._noncontact_senders:
            ...
            save_noncontact_senders(self.user, self._noncontact_senders)
        if sys.platform == 'win32' and wx.GetActiveWindow() is None:
            try:
...
class ChatDialog(wx.Dialog):
    ...
    def on_close(self, event):
        ...
        event.Skip()  # Continue with normal close processing    def on_key(self, event):
